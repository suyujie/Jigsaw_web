...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="dailyJobCompleteRateStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/daily_job_complete_rate" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										<th><tt:i18n name="jobId"/></th>
										<th><tt:i18n name="assignJobNum"/></th>
										<th><tt:i18n name="completeJobNum"/></th>
										<th><tt:i18n name="completeJobRate"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="jobCompleteRate" varStatus="status">
										<tr>
											<td>${jobCompleteRate.jobId}</td>
											<td>${jobCompleteRate.assignJobNum}</td>
											<td>${jobCompleteRate.completeJobNum}</td>
											<td><fmt:formatNumber value="${jobCompleteRate.completeJobRate}" type="percent" pattern="#0.00%"/></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/daily_job_complete_rate?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&dateTime=${result.dateTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/daily_job_complete_rate?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&dateTime=${result.dateTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=10&dateTime=${result.dateTime}"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/dailyJobCompleteRate.js" type="text/javascript"></script>
