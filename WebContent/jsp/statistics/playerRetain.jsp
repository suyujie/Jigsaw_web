...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="playerRetainStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
											<td>
												<button class="btn green" onclick="chart('${result.prArray}');"><tt:i18n name="lineChart"/></button>
											</td>
										</tr>
									</table>
								</div>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/player_retain" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										...
										<th rowspan="2"><tt:i18n name="registerNum"/></th>
										<th colspan="2"><tt:i18n name="oneDayRetain"/></th>
										<th colspan="2"><tt:i18n name="2DayRetain"/></th>
										<th colspan="2"><tt:i18n name="3DayRetain"/></th>
										<th colspan="2"><tt:i18n name="7DayRetain"/></th>
										<th colspan="2"><tt:i18n name="14DayRetain"/></th>
										<th colspan="2"><tt:i18n name="30DayRetain"/></th>
									</tr>
									<tr>
										<th><tt:i18n name="retainNum"/></th>
										<th><tt:i18n name="retainRate"/></th>
										<th><tt:i18n name="retainNum"/></th>
										<th><tt:i18n name="retainRate"/></th>
										<th><tt:i18n name="retainNum"/></th>
										<th><tt:i18n name="retainRate"/></th>
								...
								<tbody>
									<c:forEach items="${result.paging.rows}" var="playerRetain" varStatus="status">
										<tr>
											<td>${playerRetain.dateTime}</td>
											<td>${playerRetain.registerNum}</td>
											<td>${playerRetain.retainNum1}</td>
											<td><fmt:formatNumber value="${playerRetain.retention1}" type="percent" pattern="#0.00%"/></td>
											<td>${playerRetain.retainNum2}</td>
											<td><fmt:formatNumber value="${playerRetain.retention2}" type="percent" pattern="#0.00%"/></td>
											<td>${playerRetain.retainNum3}</td>
											<td><fmt:formatNumber value="${playerRetain.retention3}" type="percent" pattern="#0.00%"/></td>
											<td>${playerRetain.retainNum7}</td>
											<td><fmt:formatNumber value="${playerRetain.retention7}" type="percent" pattern="#0.00%"/></td>
											<td>${playerRetain.retainNum14}</td>
											<td><fmt:formatNumber value="${playerRetain.retention14}" type="percent" pattern="#0.00%"/></td>
											<td>${playerRetain.retainNum30}</td>
											<td><fmt:formatNumber value="${playerRetain.retention30}" type="percent" pattern="#0.00%"/></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/player_retain?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/player_retain?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=1&begin=${result.paging.begin}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/playerRetain.js" type="text/javascript"></script>
