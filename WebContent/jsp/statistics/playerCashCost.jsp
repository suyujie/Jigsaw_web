...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="playerCashCostStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
											<td>
												<button class="btn green" onclick="chart('${result.pccArray}');"><tt:i18n name="lineChart"/></button>
											</td>
										</tr>
									</table>
								</div>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/player_cash_cost" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										...
										<th colspan="7"><tt:i18n name="dayCashCost"/></th>
									</tr>
									<tr>
										<th><tt:i18n name="CashType.hireRobotCost"/></th>
										<th><tt:i18n name="CashType.robotPartUpCost"/></th>
										<th><tt:i18n name="CashType.pvpLose"/></th>
										<th><tt:i18n name="CashType.lottery"/></th>
										<th><tt:i18n name="CashType.pvpUse"/></th>
										<th><tt:i18n name="CashType.partRarityUpgrade"/></th>
										<th><tt:i18n name="CashType.bergUpgrade"/></th>
										<th><tt:i18n name="CashType.robotPartEvolution"/></th>
										<th><tt:i18n name="costAmount"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="playerCashCost" varStatus="status">
										<tr>
											<td>${playerCashCost.dateTime}</td>
											<td>${playerCashCost.todayRentRobotCost}</td>
											<td>${playerCashCost.todayRobotPartUpCost}</td>
											<td>${playerCashCost.todayPVPLose}</td>
											<td>${playerCashCost.todayLotteryCost}</td>
											<td>${playerCashCost.todayPVPUse}</td>
											<td>${playerCashCost.todayPartRarityUpgrade}</td>
											<td>${playerCashCost.todayBergUpgrade}</td>
											<td>${playerCashCost.todayRobotPartEvolution}</td>
											<td>${playerCashCost.todayCostAmount}</td>
										</tr>
									</c:forEach>
									<tr>
										<th><tt:i18n name="total"/></th>
										<td>${result.total.totalRentRobotCost}</td>
										<td>${result.total.totalRobotPartUpCost}</td>
										<td>${result.total.totalPVPLose}</td>
										<td>${result.total.totalLotteryCost}</td>
										<td>${result.total.totalPVPUse}</td>
										<td>${result.total.totalPartRarityUpgrade}</td>
										<td>${result.total.totalBergUpgrade}</td>
										<td>${result.total.totalRobotPartEvolution}</td>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/player_cash_cost?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/player_cash_cost?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=20&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/playerCashCost.js" type="text/javascript"></script>
