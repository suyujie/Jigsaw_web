...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="newPlayerAveragePayStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
											<td>
												<button class="btn green" onclick="chart('${result.npapArray}');"><tt:i18n name="lineChart"/></button>
											</td>
										</tr>
									</table>
								</div>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/new_player_avg_pay" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
											...
											<div class="col-md-2">
												<label class="col-md-3" style="margin-top:6px"><tt:i18n name="days"/></label>
												<input id="days" name="days" class="col-md-5" type="text" value="${result.days }" required />
											</div>
											<div class="col-md-2">
		                    ...
		                    <div class="row">
								<label class="col-md-3"><tt:i18n name="daysAveragePayAmount"/>： <font color="red">${result.daysAveragePayAmount/100}</font></label>
							</div>
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										<th><tt:i18n name="date"/></th>
										<th><tt:i18n name="registerPlayerNum"/></th>
										<th><tt:i18n name="totalPayAmount"/></th>
										<th><tt:i18n name="averagePayAmount"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="npap" varStatus="status">
										<tr>
											<td>${npap.dateTime}</td>
											<td>${npap.registerPlayerNum}</td>
											<td>${npap.totalPayAmount/100}</td>
											<td>${npap.averagePayAmount/100}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/new_player_avg_pay?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/new_player_avg_pay?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=4&begin=${result.paging.begin}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/newPlayerAveragePay.js" type="text/javascript"></script>
