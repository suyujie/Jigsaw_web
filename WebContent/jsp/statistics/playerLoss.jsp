...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="playerLossStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
											<td>
												<button class="btn green" onclick="chart('${result.plArray}');"><tt:i18n name="lineChart"/></button>
											</td>
										</tr>
									</table>
								</div>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/player_loss" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										...
										<th rowspan="2"><tt:i18n name="todayLoginNum"/></th>
										<th colspan="2"><tt:i18n name="dayLoss"/></th>
										<th colspan="2"><tt:i18n name="weekLoss"/></th>
										<th colspan="2"><tt:i18n name="monthLoss"/></th>
									</tr>
									<tr>
										<th><tt:i18n name="nextDayReloginNum"/></th>
										<th><tt:i18n name="dayLossRate"/></th>
										<th><tt:i18n name="nextWeekReloginNum"/></th>
										<th><tt:i18n name="weekLossRate"/></th>
										<th><tt:i18n name="nextMonthReloginNum"/></th>
										<th><tt:i18n name="monthLossRate"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="playerLoss" varStatus="status">
										<tr>
											<td>${playerLoss.dateTime}</td>
											<td>${playerLoss.todayLoginNum}</td>
											<td>${playerLoss.nextDayReloginNum}</td>
											<td><fmt:formatNumber value="${playerLoss.dayLossRate}" type="percent" pattern="#0.00%"/></td>
											<td>${playerLoss.nextWeekReloginNum}</td>
											<td><fmt:formatNumber value="${playerLoss.weekLossRate}" type="percent" pattern="#0.00%"/></td>
											<td>${playerLoss.nextMonthReloginNum}</td>
											<td><fmt:formatNumber value="${playerLoss.monthLossRate}" type="percent" pattern="#0.00%"/></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/player_loss?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/player_loss?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=0&begin=${result.paging.begin}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/playerLoss.js" type="text/javascript"></script>
