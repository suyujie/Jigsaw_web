...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="playerGoldGetStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
											<td>
												<button class="btn green" onclick="chart('${result.pggArray}');"><tt:i18n name="lineChart"/></button>
											</td>
										</tr>
									</table>
								</div>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/player_gold_get" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										...
										<th colspan="9"><tt:i18n name="dayGoldGet"/></th>
									</tr>
									<tr>
										<th><tt:i18n name="GoldType.managerGive"/></th>
										<th><tt:i18n name="GoldType.buy"/></th>
										<th><tt:i18n name="GoldType.missionGet"/></th>
										<th><tt:i18n name="GoldType.taskGet"/></th>
										<th><tt:i18n name="GoldType.dailyJobGet"/></th>
										<th><tt:i18n name="GoldType.tutorialGet"/></th>
										<th><tt:i18n name="GoldType.noticeGive"/></th>
										<th><tt:i18n name="GoldType.pointGive"/></th>
										<th><tt:i18n name="GoldType.initGive"/></th>
										<th><tt:i18n name="GoldType.handbookReward"/></th>
										<th><tt:i18n name="GoldType.compensate"/></th>
										<th><tt:i18n name="GoldType.monthCard"/></th>
										<th><tt:i18n name="GoldType.firstBuyDouble"/></th>
										<th><tt:i18n name="getAmount"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="playerGoldGet" varStatus="status">
										<tr>
											<td>${playerGoldGet.dateTime}</td>
											<td>${playerGoldGet.todayManagerGive}</td>
											<td>${playerGoldGet.todayBuy}</td>
											<td>${playerGoldGet.todayMissionGet}</td>
											<td>${playerGoldGet.todayTaskGet}</td>
											<td>${playerGoldGet.todayDailyJobGet}</td>
											<td>${playerGoldGet.todayToturialGet}</td>
											<td>${playerGoldGet.todayNoticeGive}</td>
											<td>${playerGoldGet.todayPointGive}</td>
											<td>${playerGoldGet.todayInitGive}</td>
											<td>${playerGoldGet.todayHandbookReward}</td>
											<td>${playerGoldGet.todayCompensate}</td>
											<td>${playerGoldGet.todayMonthCard}</td>
											<td>${playerGoldGet.todayFirstBuyDouble}</td>
											<td>${playerGoldGet.todayGetAmount}</td>
										</tr>
									</c:forEach>
									<tr>
										<th><tt:i18n name="total"/></th>
										<td>${result.total.totalManagerGive}</td>
										<td>${result.total.totalBuy}</td>
										<td>${result.total.totalMissionGet}</td>
										<td>${result.total.totalTaskGet}</td>
										<td>${result.total.totalDailyJobGet}</td>
										<td>${result.total.totalToturialGet}</td>
										<td>${result.total.totalNoticeGive}</td>
										<td>${result.total.totalPointGive}</td>
										<td>${result.total.totalInitGive}</td>
										<td>${result.total.totalHandbookReward}</td>
										<td>${result.total.totalCompensate}</td>
										<td>${result.total.totalMonthCard}</td>
										<td>${result.total.totalFirstBuyDouble}</td>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/player_gold_get?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/player_gold_get?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=17&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/playerGoldGet.js" type="text/javascript"></script>
