...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="playerOnlineStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
											<td>
												<button class="btn green" onclick="chart('${result.poArray}');"><tt:i18n name="lineChart"/></button>
											</td>
										</tr>
									</table>
								</div>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/player_online" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										...
										<th colspan="24"><tt:i18n name="concurrentPlayers"/></th>
										<th rowspan="2"><tt:i18n name="acu"/></th>
										<th rowspan="2"><tt:i18n name="pcu"/></th>
									</tr>
									<tr>
										<th><tt:i18n name="0h"/></th>
										<th><tt:i18n name="1h"/></th>
										<th><tt:i18n name="2h"/></th>
										<th><tt:i18n name="3h"/></th>
										<th><tt:i18n name="4h"/></th>
										<th><tt:i18n name="5h"/></th>
										<th><tt:i18n name="6h"/></th>
										<th><tt:i18n name="7h"/></th>
										<th><tt:i18n name="8h"/></th>
										<th><tt:i18n name="9h"/></th>
										<th><tt:i18n name="10h"/></th>
										<th><tt:i18n name="11h"/></th>
										<th><tt:i18n name="12h"/></th>
										<th><tt:i18n name="13h"/></th>
										<th><tt:i18n name="14h"/></th>
										<th><tt:i18n name="15h"/></th>
										<th><tt:i18n name="16h"/></th>
										<th><tt:i18n name="17h"/></th>
										<th><tt:i18n name="18h"/></th>
										<th><tt:i18n name="19h"/></th>
										<th><tt:i18n name="20h"/></th>
										<th><tt:i18n name="21h"/></th>
										<th><tt:i18n name="22h"/></th>
										<th><tt:i18n name="23h"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="playerOnline" varStatus="status">
										<tr>
											<td>${playerOnline.dateTime}</td>
											<c:forEach items="${playerOnline.hoursACUMap}" var="hour">
												<td>${hour.value}</td>
											</c:forEach>
											<td>${playerOnline.acu}</td>
											<td>${playerOnline.pcu}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/player_online?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/player_online?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=5&begin=${result.paging.begin}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/playerOnline.js" type="text/javascript"></script>
