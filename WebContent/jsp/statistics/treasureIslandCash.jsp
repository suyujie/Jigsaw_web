...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="treasureIslandCashStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/treasure_island_cash" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										...
										<th><tt:i18n name="getCashNum"/></th>
										<th><tt:i18n name="resetTimes"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="treasureIslandCash" varStatus="status">
										<tr>
											<td>${treasureIslandCash.dateTime}</td>
											<td>${treasureIslandCash.challengePlayers}</td>
											<td>${treasureIslandCash.dayActivePlayers}</td>
											<td><fmt:formatNumber value="${treasureIslandCash.challengeRate}" type="percent" pattern="#0.00%"/></td>
											<td>${treasureIslandCash.getCashNum}</td>
											<td>${treasureIslandCash.resetTimes}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/treasure_island_cash?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/treasure_island_cash?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&beginTime=${result.beginTime}&endTime=${result.endTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
	...
	<script src="/robot_web/js/statistics/treasureIslandCash.js" type="text/javascript"></script>
