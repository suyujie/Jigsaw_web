...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="deviceRangeRateStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
		                        <div class="note note-success">
									<form id="form1" action="/robot_web/statistics/device_range_rate" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-1">
												<h4><font color="green"><tt:i18n name="searchCriteria"/></font></h4>
											</div>
											<div class="col-md-3">
												<input type="hidden" id="selectedDeviceType" value="${result.deviceType}" />
												<label class="col-md-3 control-label"><tt:i18n name="deviceType"/></label>
												<select id="deviceType" name="deviceType" class="col-md-7">
													<option value="0"><tt:i18n name="phoneModel"/></option>
													<option value="1"><tt:i18n name="screenResolution"/></option>
													<option value="2"><tt:i18n name="availableMemory"/></option>s
													<option value="3"><tt:i18n name="networkEnvironment"/></option>
													<option value="4"><tt:i18n name="language"/></option>
													<option value="5"><tt:i18n name="country"/></option>
												</select>
											</div>
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										...
										<th><tt:i18n name="deviceName"/></th>
										<th><tt:i18n name="deviceNum"/></th>
										<th><tt:i18n name="totalNum"/></th>
										<th><tt:i18n name="rangeRate"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="drr" varStatus="status">
										<tr>
											<td style="display:none;"></td>
											<td>${drr.deviceName }</td>
											<td>${drr.deviceNum}</td>
											<td>${drr.totalNum}</td>
											<td><fmt:formatNumber value="${drr.rangeRate}" type="percent" pattern="#0.00%"/></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/device_range_rate?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&deviceType=${result.deviceType }"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/device_range_rate?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&deviceType=${result.deviceType }"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=21&begin=${result.paging.begin}&rowNum=${result.paging.rowNum}&deviceType=${result.deviceType }"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/deviceRangeRate.js" type="text/javascript"></script>
