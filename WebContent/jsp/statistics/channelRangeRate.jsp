...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="channelRangeRateStatistics"/></div>
							<div class="actions">
								<div class="btn-group">
									<table>
										<tr>
						...
						<div class="portlet-body">
							<div class="table-toolbar">
								<div class="note note-success">
									<form id="form1" action="/robot_web/statistics/channel_range_rate" method="post" style="margin:0 0 0 0">
										<div class="row">
											<div class="col-md-2">
							...
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										<th><tt:i18n name="channelName"/></th>
										<th><tt:i18n name="channelNum"/></th>
										<th><tt:i18n name="totalNum"/></th>
										<th><tt:i18n name="rangeRate"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="crr" varStatus="status">
										<tr>
											<td>${crr.channelName }</td>
											<td>${crr.channelNum}</td>
											<td>${crr.totalNum}</td>
											<td><fmt:formatNumber value="${crr.rangeRate}" type="percent" pattern="#0.00%"/></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<tr>
										<td>
											<c:if test="${result.paging.page > 1}">
												<a id="previous" href="/robot_web/statistics/channel_range_rate?begin=${result.paging.begin-result.paging.rowNum}&rowNum=${result.paging.rowNum}&dateTime=${result.dateTime}"><tt:i18n name="previousPage"/></a>&nbsp;&nbsp;
											</c:if>
											<c:if test="${result.paging.page < result.paging.total}">
												<a id="next" href="/robot_web/statistics/channel_range_rate?begin=${result.paging.end+1}&rowNum=${result.paging.rowNum}&dateTime=${result.dateTime}"><tt:i18n name="nextPage"/></a>&nbsp;&nbsp;&nbsp;
											</c:if>
										</td>
										<td>
										...
										<td>
											<button id="export" value="/robot_web/export_xls?fileTag=22&begin=${result.paging.begin}&rowNum=${result.paging.rowNum}"><tt:i18n name="export"/></button>
										</td>
									</tr>
								</c:if>
							</table>
	...
	<script src="/robot_web/js/statistics/channelRangeRate.js" type="text/javascript"></script>
