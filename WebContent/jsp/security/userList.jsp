...
<body class="" style="overflow-x:hidden">
				...
				<div class="col-md-12">
					<div class="portlet box blue" >
						<div class="portlet-title">
							<div class="caption"><tt:i18n name="userList"/></div>
							<div class="actions">
								<a class="btn default green" onclick="load('/robot_web/user/add')" data-target="#ajax" data-toggle="modal">Add New <i class="icon-plus"></i></a>
							</div>
						</div>
						<div class="portlet-body">
							<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
								<thead>
									<tr>
										<th><tt:i18n name="username"/></th>
										<th><tt:i18n name="password"/></th>
										<th><tt:i18n name="role"/></th>
										<th><tt:i18n name="operation"/></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${result.paging.rows}" var="user" varStatus="status">
										<tr>
											<td>${user.username}</td>
											<td>${user.password}</td>
											<td>${user.role}</td>
											<td>
												<c:if test="${user.username!='admin'}">
													<a href="/robot_web/user/delete?username=${user.username}" class="button button-rounded button-flat-primary button-tiny"><tt:i18n name="delete"/></a>
													<a onclick="load('/robot_web/user/edit?username=${user.username}')" data-target="#ajax" data-toggle="modal" class="button button-rounded button-flat-primary button-tiny"><tt:i18n name="edit"/></a>
												</c:if>
											</td>
										</tr>
									</c:forEach>
	...
	<script src="/robot_web/js/security/userList.js" type="text/javascript"></script>
